  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const allRooms: RoomCardData[] = [];
        for (const properti of propertiList) {
          try {
            const res = await fetch(`/api/properti/${properti.id}/kamar`, {
              headers: {
            ...
            if (data.status === "success" && data.data) {
              const kamarList: KamarData[] = data.data;
            ...
            console.error(`Failed to fetch rooms for ${properti.id}:`, err);
          }
        }
